...
// Append one post to an existing campaign (the editor's "Add to campaign…" flow).
async function addPost(id, post) {
  const rec = await get(id);
  rec.posts.push(wrapPosts([post])[0]);
  return save(rec);
}
...
module.exports = { list, get, create, update, remove, addPost, addFeedback, setStatus, setPost, applyRevision, pendingFeedback, POST_STATUSES };
