...
- Tokens must use EXACTLY the token names the chosen slide declares. Fill every non-optional token.
  Set lever tokens (theme/align/panel/surface/motif/lockup) explicitly from their allowed values.
  carousel-journal slides that stamp the logo (cover, intro, interior, closing) default lockup
  to wordmark (Fig & Bloom only). For Moment Makers is a lens, not a slogan — it must not sit
  on the image. Only set lockup=full if the brief asks for the historical lockup.
