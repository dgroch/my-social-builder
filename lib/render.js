...
function assembleSlide(schema, design, slideId, tokens, ratio, pos) {
  ...
  if (v.theme != null) {
    ...
    const colour = (theme === 'dark') ? 'white' : 'black';
    // lockup lever (carousel-journal cover): wordmark = Fig & Bloom only;
    // full = historical lockup with For Moment Makers. Slides that don't
    // declare the lever leave `lockup` unset and keep logo_h_*.png.
    // A declared lever defaults to its first enum value (wordmark) before
    // we get here, so omitting it on the journal cover is wordmark.
