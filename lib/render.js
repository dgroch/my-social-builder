...
function assembleSlide(schema, design, slideId, tokens, ratio, pos) {
  ...
  // theme lever → computed class. Applies to any slide that declares a `theme`
  // token (previously hard-coded to slide === 'cover'); the manifest's per-slide
  ...
  // lockup lever (carousel-journal cover / intro / interior / closing):
  // wordmark = Fig & Bloom only; full = historical lockup with For Moment Makers.
  // Slides that don't declare the lever leave `lockup` unset and keep logo_h_*.png.
  // A declared lever defaults to its first enum value (wordmark) before we get
  // here, so omitting it on those journal slides is wordmark.
  // Colour follows theme when present (dark → white); otherwise black (intro / interior).
  if (v.theme != null || v.lockup != null) {
    const colour = (v.theme === 'dark') ? 'white' : 'black';
    const lockup = (v.lockup === 'wordmark') ? 'wordmark' : 'full';
    v.LOGO_FILE = (lockup === 'wordmark')
      ? ('logo_h_' + colour + '_wordmark.png')
      : ('logo_h_' + colour + '.png');
  }
