...
function assembleSlide(schema, design, slideId, tokens, ratio) {
  ...
  // cover theme lever → computed class + logo colour (not author-set tokens)
  if (slideId === 'cover') {
...
async function renderPost(post, opts) {
  ...
  try {
    ...
    for (const s of post.slides) {
      ...
      await page.close();
      slices.push({ index: i, slide: s.slide, w, h, pngBase64: png.toString('base64'), unfilled: leftover });
    }
  } finally {
    await browser.close();
  }
