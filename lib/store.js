...
// Read the chunked Data property back. Fast path: the page object inlines up to 25
// rich_text items (~47KB of record) — no extra call. Only beyond that do we page
// through the property-item endpoint, with diagnostics on failure (production showed
// it under-returning; the inline path is the proven one).
async function readData(page) {
  const prop = page.properties['Data'];
  const inline = prop.rich_text || [];
  if (inline.length > 0 && inline.length < 25) {
    return JSON.parse(inline.map(t => (t.plain_text != null ? t.plain_text : (t.text && t.text.content) || '')).join(''));
  }
  let text = '', cursor, pages = 0, items = 0;
  do {
    const r = await napi('GET', '/pages/' + page.id + '/properties/' + encodeURIComponent(prop.id) +
      '?page_size=100' + (cursor ? '&start_cursor=' + encodeURIComponent(cursor) : ''));
    // paginated types return {object:'list', results:[...]}; tolerate a bare property_item too
    const results = r.results || (r.rich_text ? [r] : []);
    for (const item of results) text += (item.rich_text && (item.rich_text.plain_text != null ? item.rich_text.plain_text : (item.rich_text.text && item.rich_text.text.content))) || '';
    items += results.length; pages += 1;
    cursor = r.has_more ? r.next_cursor : null;
  } while (cursor);
  try { return JSON.parse(text); }
  catch (e) {
    throw new Error('Notion Data read incomplete — property-item endpoint returned ' + items +
      ' item(s) across ' + pages + ' call(s), ' + text.length + ' chars (inline had ' + inline.length + '): ' + e.message);
  }
}
